...
In addition, the following representation is often used when it comes to using the function `semPaths()`. There is a weighting of the edges depending on the standardized parameter estimates. 
...
## Sampling Weights
With lavaan version 0.6-3 there are two ways for weighting. On the one hand, you can use the package `lavaan.survey`for taking sampling weights into account. On the other hand, it's possible to specify the sampling weights within the `cfa()` function. 
...
surveyfit <- lavaan.survey(lavaan.fit = weighted_fit,
